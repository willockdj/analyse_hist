...
/***********************************************************************/
/* routine to rotate a vector co-ordinates around a given vector unit  */
/* vector by a prescribed amount (theta!)                              */
...
/* perform the rotation */
  ...
  for (iloop= 0; iloop <= num_vecs; iloop++)
  {
     for (icoloum=0; icoloum< 3; icoloum++)
     {
        ...
        hold_coord[icoloum]+= quarternion[3*icoloum]*(*p_this_vec);
        hold_coord[icoloum]+= quarternion[3*icoloum+1]*(*(p_this_vec+1));
        hold_coord[icoloum]+= quarternion[3*icoloum+2]*(*(p_this_vec+2));
     } 
     *p_this_vec     = hold_coord[0];
     *(p_this_vec+1) = hold_coord[1];
     *(p_this_vec+2) = hold_coord[2];
     p_this_vec += 3; 
  }
