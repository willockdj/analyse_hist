...
EXTERNAL int num_quatric_streches;
...
#ifdef MAIN
EXTERNAL int max_mopac_atoms =999;  /*silly until we work out what the max is*/
#else
...
EXTERNAL links link_atoms[MAXATOMS];      /* struct with frag-frag bond list */
EXTERNAL bond forbidden_bond[MAXATOMS];   /* struct for forbidden bonds */
