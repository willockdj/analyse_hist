...
void centre_of_bscat(double *p_c_of_b, double *p_total_bscat, atom *p_molecule,
       ...
       for (this_atom=0; this_atom <= num_atoms; this_atom++)
         {
            ...
            *p_c_of_b     += bfact * (p_atom->x);
            *(p_c_of_b+1) += bfact * (p_atom->y);
            *(p_c_of_b+2) += bfact * (p_atom->z);
...
/* normalise to total mass */
   ...
   for (icomp=0; icomp < 3; icomp++)
                        *(p_c_of_b+icomp)= *(p_c_of_b+icomp) / *p_total_bscat;
