...
double average_rgyr(double *a, int start, int end);
...
twoD_monitors monit_bnds;

int do_this;
int num_bins1, num_bins2, num_bins, ineigh, this_neigh, neigh_index;
...
/***********************************************/
/** Glib : Variables for radius of gyration ****/
/***********************************************/
double av_rgyr;
double st_rgyr;
int start_rgyr;
...
for (iloop=0; iloop < 10; iloop++)
  {
    ...
    anal_flags[iloop].rgyr            = FALSE;
    anal_flags[iloop].bonds           = FALSE;
    anal_flags[iloop].drawline        = FALSE;
    anal_flags[iloop].drawellipse     = FALSE;
...
good_read= read_input( input_fp, &title[0], &history_file[0],
                       ...
                       &anal_flags[0], 
                       &have_field, &num_molecules, &num_this_mol[0], 
                       &use_type[0], &num_to_use, &num_anal_flags, 
                       &cutoff, &have_config, &have_pdb,
                       &bisector_atoms[0], 
                       &bisect_disp, 
                       &min_corr_time, &max_corr_time, &min_msd_time, 
                       &max_msd_time, &time_step, &have_rdf, 
                       &have_out, &want_raw, &skip_out, 
                       &monit_bnds, 
                       &num_std_bnds, &monitor_set[0], &monitor_set_angle[0], 
                       &monitor_set_dihedral[0], &num_monit, &num_monit_angle, 
                       &num_monit_dihedral, &num_limit_bond, &num_limit_angle, 
                       &num_limit_dihedral, 
                       &xdatcar_file[0], &have_history, &have_xdatcar,
                       &poscar_file[0], &potcar_file[0], &have_poscar, 
                       &have_potcar, &winref, &need_draw, 
                       &rgyr_av_start,  
                       &elli_start, &elli_atype[0] );
...
else if  (!have_xdatcar && !have_history && !have_config && !have_pdb)
  {
    printf("ERROR: None of HISTORY, XDATCAR, PDB or CONFIG files are defined, need a trajectory or structure file!\n");
    exit(0);
  }
    ...
    if (anal_flags[ianal].msd)
                {
                   ...
                   if (time_step < 0.0) 
                   ...
                   else
                     {
                         ...
                         printf("Min time for msd %10.6f, max time %10.6f picoseconds\n",
   ...
   if (anal_flags[ianal].bonds)
     {
       ...
       printf("For bonds involving only: ");
       printf("%s %s\n", monit_bnds.atom1,  monit_bnds.atom2);

       printf("min dist : %10.6f  max dist : %10.6f delta: %10.6f\n",
                     monit_bnds.min_limit1, monit_bnds.max_limit1, monit_bnds.delta1);

       printf("min angle: %10.6f  max angle: %10.6f delta: %10.6f\n", 
                     monit_bnds.min_limit2, monit_bnds.max_limit2, monit_bnds.delta2);
       printf("\n");
/*** define range of bonds to look for ***/

       num_bins1 = (monit_bnds.max_limit1 -  monit_bnds.min_limit1)/ monit_bnds.delta1;
       num_bins2 = (monit_bnds.max_limit2 -  monit_bnds.min_limit2)/ monit_bnds.delta2;

       printf("Will require %d bins for dist by %d for angle\n", num_bins1, num_bins2); 
       if (num_bins1 > MAX_BINS || num_bins2 > MAX_BINS)
         {
           printf("ERROR: Current maximum number of bins = %d\n",
                      MAX_BINS);
           exit(0);
         }
      printf("Malloc the space:\n");
      printf("creating an array of pointers for the dist part\n");
      dist_twoD_bonds= malloc( num_bins1 * sizeof(double *));
      if (dist_twoD_bonds == NULL)
        {
           printf("ERROR: Failure to get enough memory for bonds twoD pointers\n");
           exit(0);
        }

      printf("allocating enough memory for the angle data for each pointer of the dist part\n");

      for (iii=0; iii< num_bins1; iii++)
        {
           dist_twoD_bonds[iii] = malloc(num_bins2*sizeof(double));

           for (ibin = 0; ibin < num_bins2; ibin++) dist_twoD_bonds[iii][ibin]=0.0;
        }
...
/***** 1 signifies HISTORY begins with timestep line ***/
          ...
          { 
            sprintf(title_store, "No pdb title read");
            printf(" PDB file title : %s\n", title_store);
...
/*** First part of normalisation factor for g(r) plots *******************/
	...
	/***** Read a frame and process it, frame reader returns -10 at end of file ! ****/
	/*********************************************************************************/
	/***** Adapting to allow pdb file reading, May 2013 Dave Willock *****************/
	/*********************************************************************************/
      ...
      for (iii=0; iii<=num_monit; iii++)
         {
          ...
          for (icheck=0; icheck<=max_atom_index; icheck++)                  
             ...
             /*Note loop index here should stop double counting ***/
			         ...
			         if (anal_flags[ianal].msd && ( start_mol > 0 || num_molecules == 1 ) )
			           {
                                      ...
                                      printf("Calculating centre of mass for msd calculation with %d atoms\n", 
			              ...
			              centre_of_mass(&c_of_m[0], &total_mass, &molecule[start_mol], 
                                      ...
                                      if (num_cofm_list >= MAX_MSD_LIST)
                                        { 
                                          printf("ERROR: Too many frames for the MSD list, currently MAX_MSD_LIST= %d\n",
                                                                MAX_MSD_LIST);
                                          exit(0);
                                        }
...
*/
	...
	/*********************************************/
                                  ...
                                  for (iloop=start_mol; iloop <= demarcation[index].end; iloop++)
				    {
                                  ...
                                  if (last_in_frame && this_molecule == num_this_mol[itype]-1 ) 
                                     {
                                       printf("This is the last instance of the last type of molecule ");
                                       printf("in this frame so writing frame\n");
...
/*** Analysis added for co-crystals *****/
                      ...
                      if (anal_flags[ianal].bonds)
                        ...
                        do_this = FALSE;
                        if (strcmp(molecule[iatom].label, monit_bnds.atom1)==0) 
                            {
                                 do_this = TRUE;
                                 printf("Considering atom %d because its label is %s\n", molecule[iatom].label);
                            }
                     if (do_this) 
                       {
...
/**** Write window information for this set of molecules now ****/
             if (anal_flags[ianal].windows)
               {
                  if (use_for_wins[itype])
                    {
                      if (start_frame && start_type) 
                                         write_window_csv_titles(windows_fp,
                                                                 num_molecules, winsets.num_windows,
                                                                 winref.num_windows, &use_for_wins[0]);
        /*********************************************/
        /*** Only include flagged molecule types   ***/
        /*********************************************/
                 ...
                 if (last_in_frame) fprintf(windows_fp, "\n");
               }

/*** Average values across molecules for this frame as required ****/
                   if (anal_flags[ianal].rgyr) 
                     {
                        rgyr_molav[frame_index] = 0.0;
...
/**** Glib starts                                                                           ***/
                  ...
                  printf("Setting start time for average : %10.6f\n", rgyr_av_start);
                  start_rgyr=0;
                  for (iii=0; iii<=frame_index; iii++)
                  {
                    printf("Comparing time_now, %10.6f with required start %10.6f\n", time_now[iii]
                                                                                    , rgyr_av_start);
                    if (time_now[iii] >= rgyr_av_start)
                    {
                            start_rgyr=iii;
                            break;
                    }
                  ...
                  printf("Going to average_rgyr with start index %d\n", start_rgyr);
                  av_rgyr = average_rgyr( &rgyr_molav[0], start_rgyr, frame_index);
                  printf("Back from to average_rgyr\n");
                  printf("Average radius of gyration from frame %d to frame %d : %10.6f\n", start_rgyr,
                                                                                            frame_index,
                                                                                            av_rgyr);


                  printf("Going to st_rgyr\n");
                  st_rgyr = std_rgyr( &rgyr_molav[0], start_rgyr, frame_index);
                  printf("Back from to st_rgyr\n");
                  printf("Standart deviation of radius of gyration : %10.6f\n", st_rgyr);
